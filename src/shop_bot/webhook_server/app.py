...
def create_webhook_app(bot_controller_instance):
    ...
    def users_table_partial():
        ...
        # if args missing, try to parse them from referrer so AJAX refresh keeps current page
        if page is None or per_page is None or q is None:
            ref = request.referrer or ''
            if ref:
                import urllib.parse as _u
                try:
                    parts = _u.urlparse(ref)
                    qs = _u.parse_qs(parts.query)
                    if page is None and 'page' in qs:
                        try: page = int(qs.get('page',[1])[0])
                        except: page = 1
                    if per_page is None and 'per_page' in qs:
                        try: per_page = int(qs.get('per_page',[25])[0])
                        except: per_page = 25
                    if q is None and 'q' in qs:
                        q = qs.get('q',[''])[0]
                except Exception:
                    pass
        page = page or 1
        ...
        for user in users:
            ...
            try:
                ...
                user['balance'] = 0.0
            user['active_keys'] = int(keys_counts.get(uid, 0))
    ...
    @login_required
    def admin_keys_page():
        # Paginated admin keys page. Supports query ?page=&per_page=&q=
        page = request.args.get('page', 1, type=int)
        ...
        if q:
            qlow = q.lower()
            def match(k):
                for field in ('key_email','host_name','comment','remnawave_user_uuid'):
                    val = str(k.get(field,'') or '')
                    if qlow in val.lower():
                        return True
                # also check user id and key id
                if qlow in str(k.get('user_id','')) or qlow in str(k.get('key_id','')):
                    return True
                return False
        ...
        return render_template('admin_keys.html',
                               ...
                               keys_total=total,
                               keys_page=page,
                               keys_per_page=per_page,
                               keys_total_pages=total_pages,
                               keys_query=q,
                               **common_data)
    ...
    def admin_keys_table_partial():
        # Return only the table body for ajax refresh. Will respect page, per_page, q from args or referrer.
        page = request.args.get('page', type=int)
        per_page = request.args.get('per_page', type=int)
        q = request.args.get('q')

        # If page/per_page/q not provided, try to extract from referrer (so client-side refresh keeps state)
        if page is None or per_page is None or q is None:
            ref = request.referrer or ''
            if ref:
                import urllib.parse as _u
                try:
                    ...
                    if per_page is None and 'per_page' in qs:
                        try: per_page = int(qs.get('per_page',[30])[0])
                        except: per_page = 30
                    if q is None and 'q' in qs:
                        q = qs.get('q',[''])[0]
                except Exception:
        ...
        # pass pagination context so template can render correctly if needed
        return render_template('partials/admin_keys_table.html', keys=page_keys, keys_total=total, keys_page=page, keys_per_page=per_page, keys_total_pages=total_pages, keys_query=q)
