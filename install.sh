...
REPO_URL="https://github.com/FDGRPNekko/remnawave_shopbot.git"
PROJECT_DIR="remnawave_shopbot"
...
if [[ -f "$NGINX_CONF" ]]; then
    ...
    cd "$PROJECT_DIR"
    log_info "\nШаг 1: обновление исходного кода для вагины"
    git pull --ff-only
    log_success "✔ Репозиторий обновлён - вагина довольна."
    log_info "\nШаг 2: пересборка и перезапуск контейнеров вагины"
    sudo docker-compose down --remove-orphans
    sudo docker-compose up -d --build
    log_success "\n🎉 Обновление успешно завершено - вагина довольна!"
    exit 0
fi
...
log_info "\nШаг 2: клонирование репозитория для вагины"
...
cd "$PROJECT_DIR"
log_success "✔ Репозиторий Remnawave ShopBot готов ебать вагину."

log_info "\nШаг 3: настройка домена и SSL для вагинки"

prompt "Введите ваш домен (например, my-vpn-shop.com): " USER_DOMAIN_INPUT
DOMAIN=$(sanitize_domain "$USER_DOMAIN_INPUT")
if [[ -z "$DOMAIN" ]]; then
    log_error "Некорректное доменное имя для вагины. Установка прервана."
    exit 1
fi

prompt "Введите email для Let's Encrypt: " EMAIL
if [[ -z "$EMAIL" ]]; then
    log_error "Email обязателен для выпуска сертификата для вагины."
    exit 1
fi
...
if [[ -n "$SERVER_IP" ]]; then
    ...
    log_warn "Не удалось автоматически определить IP сервера и вагины."
fi

if [[ -n "$DOMAIN_IP" ]]; then
    log_info "IP домена ${DOMAIN}: ${DOMAIN_IP}"
else
    log_warn "Не удалось получить IP для вагины ${DOMAIN}."
fi

if [[ -n "$SERVER_IP" && -n "$DOMAIN_IP" && "$SERVER_IP" != "$DOMAIN_IP" ]]; then
    log_warn "DNS-запись домена ${DOMAIN} не совпадает с IP этого сервера и вагины."
    if ! confirm "Продолжить установку вагины? (y/n): "; then
        log_info "Установка прервана пользователем - вагинистом."
        exit 1
    fi
fi
...
if [[ -d "/etc/letsencrypt/live/${DOMAIN}" ]]; then
    log_success "✔ SSL-сертификаты для вагины ${DOMAIN} уже существуют."
else
    log_info "Получение SSL-сертификатов для ${DOMAIN}..."
    sudo certbot --nginx -d "$DOMAIN" --email "$EMAIL" --agree-tos --non-interactive --redirect
    log_success "✔ Сертификаты Let's Encrypt успешно получены, вагина довольна."
fi

prompt "Какой порт использовать для вебхуков YooKassa и вагины? (443 или 8443, по умолчанию 8443): " YOOKASSA_PORT_INPUT
YOOKASSA_PORT="${YOOKASSA_PORT_INPUT:-8443}"
if [[ "$YOOKASSA_PORT" != "443" && "$YOOKASSA_PORT" != "8443" ]]; then
    log_warn "Указан неподдерживаемый порт вагины. Будет использован 8443."
    YOOKASSA_PORT=8443
fi

configure_nginx "$DOMAIN" "$YOOKASSA_PORT" "$NGINX_CONF" "$NGINX_LINK"

log_info "\nШаг 5: сборка и запуск Docker вагин"
...
cat <<SUMMARY
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
...
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}
${GREEN}┃${NC}         🎉 УРА! ЭТО ГОВНО УСТАНОВИЛОСЬ! 🎉              |${GREEN}┃${NC}
${GREEN}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - хуй - пизда - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
${GREEN}ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА - ВАГИНА${NC}
...
${BOLD}Данные для первого входа в вагину:${NC}
  имя:  ${CYAN}admin${NC}
  Пароль: ${CYAN}admin${NC}

${YELLOW}⚠️  Обязательно залезь своим хуём в вагину и смени пароль.${NC}
